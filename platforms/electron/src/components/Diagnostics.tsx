...
export function Diagnostics() {
  ...
  const cards = [
    ...
    { icon: Radio, title: 'Relay dan buzzer', state: online ? 'warn' as const : 'error' as const, label: online ? 'STATUS LOGIS' : 'TIDAK DIKETAHUI', description: 'Menampilkan status logis terakhir; keberhasilan fisik perlu uji langsung.', rows: [['Relay logis', online ? (data?.relay ? 'ON' : 'OFF') : 'Tidak ada heartbeat'], ['Buzzer', 'Tidak dapat dipastikan jarak jauh']] },
  ...
  const pinRows = useMemo(() => [
    ...
    ['Relay utama', `Output GPIO${gpio(data?.relay_pin, 26)}`, online ? (data?.relay ? 'LOGIS ON' : 'LOGIS OFF') : 'TIDAK DIKETAHUI', 'Firmware hanya mengetahui level output; kontak fisik memerlukan uji langsung.'],

...
function getStatusLabel(status) {
  if (status === 'DANGER' || status === 'LEAKAGE') return 'Critical — arus tinggi';
...
function renderConnectionMeta(m) {
  ...
  elEndpointBadge.className =
    'ep-badge ' +
    (b === 'LOCAL' ? 'ep-local' : b === 'FALLBACK' ? 'ep-fallback' : 'ep-cloud');
...
async function sendRelayCommand(val) {
  try {
    if (currentRole !== 'admin') {
      showToast('Akses ditolak: hanya admin yang bisa mengontrol relay.', 'error');
      return;
    }
...
function formatLogTime(raw) {
  const n = Number(raw);
  if (Number.isFinite(n) && n > 1e12) return new Date(n).toLocaleString('id-ID');
...
function startMiniLogsListener() {
  ...
  stopLogs = onValue(logsRef, (snap) => {
    ...
    elMiniLogs.innerHTML = rows
      .map(
        (r) => `<tr>
      <td>${formatLogTime(r.waktu ?? r.timestamp)}</td>
      <td>${Number(r.arus || 0).toFixed(2)} A</td>
      <td>${Number(r.tegangan || 0).toFixed(0)} V</td>
      <td><span class="status-badge status-${r.status || 'NORMAL'}">${r.status || 'NORMAL'}</span></td>
    </tr>`
      )
      .join('');
  });

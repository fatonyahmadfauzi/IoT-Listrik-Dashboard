...
echo -e "\033[1;32m      Termux / Linux / macOS - CLI v${CLI_VERSION}       \033[0m"
echo -e "\033[1;36m========================================================\033[0m"
echo ""
...
install_nodejs() {
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        ...
        if command -v brew &> /dev/null; then
            ...
            echo -e "\033[1;31m[X] Homebrew tidak ditemukan. Install Homebrew dulu:\033[0m"
    ...
    # Linux â€“ deteksi package manager
    ...
    # Tidak bisa deteksi
    echo -e "\033[1;31m[X] Tidak dapat mendeteksi package manager.\033[0m"
...
if ! command -v node &> /dev/null; then
    ...
    if ! command -v node &> /dev/null; then
        echo -e "\033[1;31m[X] Instalasi Node.js gagal. Coba install manual lalu jalankan script ini lagi.\033[0m"
        exit 1
    fi
    echo -e "\033[1;32m[OK] Node.js berhasil diinstall! ($(node -v))\033[0m"
else
    echo -e "\033[1;32m[OK] Node.js sudah terinstall ($(node -v))\033[0m"
fi

# Pengecekan Versi Node.js (Minimal v18)
NODE_VER=$(node -v | grep -oE '[0-9]+\.[0-9]+' | head -1 | cut -d. -f1)
if [ -z "$NODE_VER" ] || [ "$NODE_VER" -lt 18 ]; then
    echo -e "\033[1;31m[X] Versi Node.js terlalu lama ($(node -v)). CLI ini membutuhkan Minimal Node v18.\033[0m"
...
if ! command -v npm &> /dev/null; then
    echo -e "\033[1;31m[X] npm tidak ditemukan. Pastikan npm ikut terinstall bersama Node.js.\033[0m"
...
curl -fsSL "https://iot-listrik-dashboard.vercel.app/downloads/cli/node-source/index.js?v=${CLI_VERSION}" -o index.js
curl -fsSL "https://iot-listrik-dashboard.vercel.app/downloads/cli/node-source/package.json?v=${CLI_VERSION}" -o package.json

...
void loop() {
  ...
  String newStatus = determineStatus(state.arus, state.tegangan,
                                      rt.thresholdArus);
  bool statusChanged = (newStatus != lastStatus);

  // ── Auto-cutoff (uses rt.autoCutoffEnabled) ──────────────────
  if (rt.autoCutoffEnabled &&
  ...
  if (statusChanged) {
    ...
    if (newStatus == "LEAKAGE") buzzerBeep(5, 200, 100);
  }

  // ── Firebase write (rt.sendIntervalMs controls rate) ─────────
  if (now - lastSendMs >= rt.sendIntervalMs) {
    lastSendMs = now;
    writeMonitorData(state.arus, state.tegangan, state.status, state.relay);
  ...
  if (now - lastRelayCheckMs >= 2500) {
    ...
    if (readRelayCommand(cmdRelay) && cmdRelay != state.relay) {
      ...
      if (cmdRelay == 1 && rt.autoCutoffEnabled &&
         (state.status == "DANGER" || state.status == "LEAKAGE")) {
